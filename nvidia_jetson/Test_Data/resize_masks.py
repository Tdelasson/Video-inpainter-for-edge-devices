...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Resize mask folders (e.g. SyntheticMasks -> SyntheticMasks_256_256)."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--src-subdir",
        type=str,
        default="SyntheticMasks",
        help="Source mask subfolder under each dataset.",
    )
    parser.add_argument(
        "--dst-prefix",
        type=str,
...
def resize_masks(
    ...
    if not src_root.exists():
        raise FileNotFoundError(f"Source masks directory not found: {src_root}")
    ...
    for width, height in target_sizes:
        ...
        total_written = 0
        total_skipped = 0
        print(f"[{dataset}] Writing resized masks to {dst_root}")
        ...
        for video_dir in video_dirs:
            ...
            for src_img_path in iter_images(video_dir):
                ...
                with Image.open(src_img_path) as img:
                    # Convert to RGB or L to ensure proper format
                    if img.mode == "RGBA":
                        img = img.convert("L")
                    elif img.mode not in ("RGB", "L"):
                        img = img.convert("L")
                    # Use NEAREST for masks to preserve exact binary values
                    resized = img.resize((width, height), resample=Image.NEAREST)
                    resized.save(dst_img_path)
                total_written += 1

        print(
            f"[{dataset}] {width}x{height}: wrote {total_written} masks"
